...
def load_cleaned_data(db_path="property_data.db"):
    ...
    conn.close()
    df["amenities"] = df["amenities"].apply(lambda x: x.split(",") if isinstance(x, str) else x)
    return df
...
def find_similar_properties(df, user_size, user_price, user_location, user_amenities):
    ...
    partial_match = df_filtered[df_filtered["location"].str.contains(user_location.lower(), na=False)]
    df_filtered = pd.concat([exact_match, partial_match]).drop_duplicates(subset=["link"], keep='first')
    if df_filtered.empty:
    ...
    tfidf_matrix = vectorizer.fit_transform(df_filtered["amenities_str"].tolist() + [user_amenities_str])
    ...
    return top_results[["link", "size", "price", "location", "amenities", "description", "building_name"]]
...
if __name__ == "__main__":
    ...
    top_properties = find_similar_properties(df, user_size, user_price, user_location, user_amenities)
